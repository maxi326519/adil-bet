...
import styles from "./BetsDepositUser.module.css";
...
export default function BetsDepositUser() {
  ...
  if (
    userActivities?.Bets?.userAllBets?.length > 0 &&
    userActivities?.Deposit?.userAllDeposit?.length > 0
  ) {
    return (
      <div className={styles.card}>
        ...
        {userActivities.Bets.userAllBets.map((e) => {
          return (
            <div>
              <p className={styles.status}>Estado: {e.status}</p>
  ...
  if (
    userActivities?.Withdraw?.userAllWithdraw?.length > 0 &&
    !userActivities.Bets
  ) {
    return (
      <div className={styles.card}>
    ...
    return (
      <div>
        <p className={styles.empty}> No hay historial de esta información.</p>
      </div>
    );
  }
}
